Rem
Rem $Header: tfa/src/v2/tfa_home/resources/sql/srdc_aqhc10R2.sql /main/4 2020/12/07 21:06:51 bvongray Exp $
Rem
Rem srdc_aqhc10R2.sql
Rem
...
Rem    SQL_SOURCE_FILE: tfa/src/v2/tfa_home/resources/sql/srdc_aqhc10R2.sql
...
REM
REM This healthcheck script version 5.0 is for use on the following Oracle database only.
REM     Oracle 10gR2
REM
REM Do not use this script on Oracle 10gR1 or Oracle 11g configurations.
REM
REM
...
prompt Statistics : <a href="#Qstats">Queues</a> <a href="#subprop">Propagation</a> 
...
select SUBSCRIPTION_NAME, R.LOCATION_NAME, CONNECTION#, QOSFLAGS, TIMEOUT, STATUS, VERSION
from SYS.REG$ R, SYS.LOC$ L
WHERE R.LOCATION_NAME = L.LOCATION_NAME (+)
order by SUBSCRIPTION_NAME;

prompt

select SUBSCRIPTION_NAME, PAYLOAD_CALLBACK, NAMESPACE, PRESENTATION, USER_CONTEXT, USER#
from SYS.REG$ R
order by SUBSCRIPTION_NAME;
...
select s.job, s.broken, s.instance instance_id, r.instance running_instance, r.sid, s.failures, s.last_date, s.last_sec, s.this_date, s.this_date
from DBA_JOBS s LEFT OUTER JOIN DBA_JOBS_RUNNING r
    ON s.job=r.job
where s.what LIKE '%register_driver%'
order by s.instance, s.job;
...
select agent_name, agent_status, agent_ping, agent_job, max_memory, max_threads, agent_start_time
...
prompt ============================================================================================
prompt                                                                     
prompt ++ QUEUE STATISTICS ++ <a href="#Top">Top</a>
prompt
prompt ============================================================================================
prompt

col queue_schema HEADING 'Owner' format a10
col qname HEADING 'Queue' format a30
col qid HEADING 'Id' format 999999999
col waiting HEADING 'Waiting'
col ready HEADING 'ready'
col total_wait HEADING 'Total|Wait'
col average_wait HEADING 'Average|Wait'

select owner||'.'||name qname, v.qid, waiting, ready, expired, total_wait, average_wait
from v$aq v, dba_queues q
where q.qid=v.qid
order by qname;
